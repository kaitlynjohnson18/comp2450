...
public:
    ...
    void clear() {
        Node* p = head_;
        while (p != nullptr) {
            Node* n = p->next; //saving next value into local pointer
            delete p; //freeing current node in the chain
            p = n; //advancing to the next node until at the end (at nullptr)
        }
        head_ = nullptr; //chain is gone  .... :_(
        size_ = 0;
        // TODO Wednesday
    }
